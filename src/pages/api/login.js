  ...
  try {
    if (req.method !== "POST") {
      res
        .status(405)
        .json({
          error: "'/login' API only accepts the POST method, not " + req.method,
        });
      return;
    }
    if (req.headers["content-type"] !== "application/json") {
      res
        ...
        .json({
          error:
            "'/login' API only accepts 'application/json' as the content type header!",
        });
      return;
    }
    ...
    console.warn(err);
    res.json({
      error: "Something went wrong when attempting to authenticate!",
    });
  }
}
